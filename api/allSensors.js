 ...
 */
async function getLogSensors(req,res){
    try{
        
        let query = constants.selectSensors; //busca el query de temperaturas
 ...
 * @param {*} res 
 */
async function getSensorsBetweenDates(req,res){
    try{
        //later: validate session and find users tasks
        let query = constants.selectSensorsByDate;
 ...
 * @param {*} res 
 */
async function insertNewSensors(req, res) {
    try {
        //later: validate session and find users tasks
        let query = constants.insertSensors;

        // Datos enviados por Arduino / ESP32
        ...
        // Parámetros en mismo orden que el INSERT
        let params = [level, temperature, light, gas, open];
...
module.exports = {getLogSensors,getSensorsBetweenDates,insertNewSensors};
